...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree){
        ...
        return Buffer.concat(
            [util.varIntBuffer(rpcData.votes.length)].concat(
                rpcData.votes.map(function (vt) {
                    return Buffer.alloc(vt.length, vt, 'hex');
                })
            )
        );
    }
    ...
    const prevBlockHash = rpcData.previousblockhash
    this.prevHashReversed = util.reverseByteOrder(Buffer.alloc(prevBlockHash.length, prevBlockHash, 'hex')).toString('hex');
    this.transactionData = Buffer.concat(rpcData.transactions.map(function(tx){
        const txData = tx.data
        return Buffer.alloc(txData.length, txData, 'hex');
    }));
