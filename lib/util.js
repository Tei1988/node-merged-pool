...
exports.addressFromEx = function(exAddress, ripdm160Key){
    try {
        ...
        var addrBase = Buffer.concat([versionByte, Buffer.alloc(ripdm160Key.length, ripdm160Key, 'hex')]);
...
exports.reverseBuffer = function(buff){
  ...
  for (var i = 0; i < len; i++) {
    reversed[len - i - 1] = buff[i]
  }
  return reversed
};

exports.reverseHex = function(hex){
    return exports.reverseBuffer(Buffer.alloc(hex.length, hex, 'hex')).toString('hex');
};
...
exports.uint256BufferFromHash = function(hex){
  return exports.reverseBuffer(hex ? Buffer.alloc(hex.length, hex, 'hex') : Buffer.alloc(32, 'hex'))
};
...
exports.varStringBuffer = function(string){
    var strBuff = Buffer.alloc(string.length, string);
...
exports.pubkeyToScript = function(key){
    ...
    pubkey[0] = 0x21;
    pubkey[34] = 0xac;
    Buffer.alloc(key.length, key, 'hex').copy(pubkey, 1);
    return pubkey;
};


exports.miningKeyToScript = function(key){
    var keyBuffer = Buffer.alloc(key.length, key, 'hex');
...
exports.bignumFromBitsHex = function(bitsString){
    var bitsBuff = Buffer.alloc(bitsString.length, bitsString, 'hex');
